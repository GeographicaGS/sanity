...
create or replace function diseasesPoxByStepsNoCumulative(start timestamp,finish timestamp,nsteps numeric) 
    returns setof tStepIntDiseases as $$
DECLARE
    r RECORD;
    nseconds int;
...
BEGIN
    ...
    vinterval := (nseconds/nsteps || ' seconds')::interval;
    
    FOR r in select * FROM generate_series(start,finish,vinterval) LOOP

        next_date := r.generate_series + vinterval;
   
        inter := (SELECT count(cartodb_id) FROM diseases_pox WHERE date_disease>=r.generate_series AND date_disease<next_date)::int;        
        
        RETURN QUERY SELECT r.generate_series::timestamp,inter;
...
create or replace function diseasesPoxByStepsNonCumulative(start timestamp,finish timestamp,nsteps numeric) 
    returns setof tStepIntDiseases as $$
DECLARE
    r RECORD;
    nseconds int;
